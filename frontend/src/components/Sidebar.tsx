    ...
    const fetchSystemInfo = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setSystemInfo({
            version: "1.0.0", // Static for now, could be fetched from API
            build: "2024.01.15", // Static for now, could be fetched from API
        ...
        <div className="text-xs text-gray-500 space-y-1">
          ...
          <div className="flex justify-between">
            ...
            <span className={`font-medium ${
              systemInfo.status === "Online" ? "text-green-600" : "text-red-600"
            }`}>
              {systemInfo.status}
            </span>
          </div>
          <div className="flex justify-between">
            <span>Mode:</span>
            <span className={`font-medium ${
              systemInfo.mode === "Learn" ? "text-blue-600" : "text-orange-600"
            }`}>
              {systemInfo.mode}
            </span>
          </div>
          <div className="pt-2 border-t border-gray-200 mt-2">
            <a 
              ...
              className="flex items-center justify-center text-xs text-blue-600 hover:text-blue-800 font-medium"
            >
