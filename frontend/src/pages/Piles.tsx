...
const QUICK_ADD_SOURCES: QuickAddSource[] = [
  // Lightweight/Moderate-Size Sources
  {
    ...
    description: 'Complete English Wikipedia with all articles and images. Essential offline knowledge base.',
    category: 'education',
    source_type: 'kiwix',
  ...
  },
  {
    ...
    description: 'Medical articles subset of Wikipedia. Ideal for health and emergency medicine.',
    category: 'medical',
    source_type: 'kiwix',
  ...
  },
  {
    ...
    description: '~60k public-domain eBooks including literature, science, and history classics.',
    category: 'books',
    source_type: 'kiwix',
  ...
  },
  {
    ...
    description: 'Simplified English Wikipedia with easier language for learning and basic reference.',
    category: 'education',
    source_type: 'kiwix',
  ...
  },
  {
    ...
    description: 'Complete English dictionary and thesaurus with definitions, pronunciations, and etymology.',
    category: 'education',
    source_type: 'kiwix',
  ...
  },
  {
    ...
    description: 'Comprehensive travel guides for destinations worldwide with maps and practical information.',
    category: 'travel',
    source_type: 'kiwix',
  ...
  // Test with a smaller, reliable file
  {
    ...
    description: 'A small test file to verify download functionality works correctly.',
    category: 'general',
    source_type: 'http',
...
export function Piles() {
  ...
  const [validationStatus, setValidationStatus] = useState<Record<string, { valid: boolean; message: string; checking: boolean }>>({})
  ...
  const handleAddPile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const validationResponse = await fetch('http://localhost:8080/api/v1/piles/validate-url', {
        method: 'POST',
        body: formData,
      })
      ...
      if (validationResponse.ok) {
        ...
        if (!validationResult.valid) {
          alert(`Cannot add pile: ${validationResult.message}\n\nURL: ${newPile.source_url}`)
          return
        }
        
        // Show file size if available
        if (validationResult.file_size) {
          const sizeMB = (validationResult.file_size / (1024 * 1024)).toFixed(1)
  ...
  const loadPiles = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to load piles:', response.status, response.statusText)
      }
    } catch (error) {
  ...
  const handleDownload = async (pile: Pile) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/v1/piles/${pile.id}/download-source`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDelete = async (pile: Pile) => {
    ...
    if (!confirm(`Are you sure you want to delete "${pile.display_name}"? This will also delete the downloaded file.`)) {
      return;
    }
    
    try {
      const response = await fetch(`http://localhost:8080/api/v1/piles/${pile.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const validateQuickAddUrl = async (sourceId: string, url: string) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/v1/piles/validate-url', {
        method: 'POST',
        body: formData,
      })
    ...
    <div>
      ...
      {showQuickAdd && (
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <h2 className="text-lg font-medium text-gray-900 mb-4">Quick Add - Popular Content Sources</h2>
          <p className="text-sm text-gray-600 mb-4">
            Add popular offline content repositories with one click. These are curated sources that work well with BabylonPiles.
          </p>
            ...
            {QUICK_ADD_SOURCES.map((source) => (
              <div key={source.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-medium text-gray-900 text-sm">{source.display_name}</h3>
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-1 text-xs rounded ${getDifficultyColor(source.difficulty)}`}>
                      {source.difficulty}
                    </span>
                    {validationStatus[source.id] && (
                      <span className={`px-2 py-1 text-xs rounded ${
                        validationStatus[source.id].checking ? 'bg-blue-100 text-blue-800' :
                        validationStatus[source.id].valid ? 'bg-green-100 text-green-800' :
                        'bg-red-100 text-red-800'
                      }`} title={validationStatus[source.id].message}>
                        {validationStatus[source.id].checking ? 'Checking...' :
                         validationStatus[source.id].valid ? '✓ Valid' : '✗ Invalid'}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-2 mb-3">
                  <span className={`px-2 py-1 text-xs rounded ${getCategoryColor(source.category)}`}>
                    {source.category}
                  </span>
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => validateQuickAddUrl(source.id, source.source_url)}
      ...
      {showAddForm && (
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <h2 className="text-lg font-medium text-gray-900 mb-4">Add New Pile</h2>
          <form onSubmit={handleAddPile} className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('source_type', e.target.value as 'kiwix' | 'http' | 'torrent')}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
            ...
            <div>
              ...
              <div className="mt-1 text-sm text-gray-500">
                <p className="font-medium">Examples for {newPile.source_type}:</p>
                <ul className="list-disc list-inside mt-1">
                  {getSourceTypeExamples(newPile.source_type).map((example, index) => (
        ...
        {loading ? (
          ...
          <div className="p-6">
            ...
            <p className="text-sm text-gray-400 mt-2">
              Use "Quick Add" to add popular content sources or "Add Pile" for custom sources.
            </p>
          </div>
        ) : (
          <div className="p-6">
              ...
              {piles.map((pile, index) => (
                <div key={index} className="border rounded-lg p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-medium text-gray-900">{pile.display_name}</h3>
                      <p className="text-sm text-gray-500">{pile.name}</p>
                      <p className="text-sm text-gray-600 mt-1">{pile.description}</p>
                      <div className="flex items-center mt-2 space-x-2">
                        <span className={`px-2 py-1 text-xs rounded ${
                          pile.source_type === 'kiwix' ? 'bg-blue-100 text-blue-800' :
                          pile.source_type === 'http' ? 'bg-green-100 text-green-800' :
                          'bg-purple-100 text-purple-800'
                        }`}>
                          {pile.source_type.toUpperCase()}
                        </span>
                        <span className={`px-2 py-1 text-xs rounded ${getCategoryColor(pile.category)}`}>
                          {pile.category}
                        </span>
                        {pile.tags && pile.tags.map((tag, tagIndex) => (
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <div className="w-24 bg-gray-200 rounded-full h-2">
                              <div 
                                ...
                                style={{ width: `${(pile.download_progress || 0) * 100}%` }}
                              ></div>
                            </div>
                            <span className="text-xs text-gray-600">
                              {Math.round((pile.download_progress || 0) * 100)}%
                            </span>
                          </div>
                        ) : pile.file_path ? (
                          <div className="flex items-center space-x-2">
