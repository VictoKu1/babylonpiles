...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        default="sqlite:///./babylonpiles.db",
        env="DATABASE_URL"
    )
    ...
    # Storage
    data_dir: str = Field(
        default="/mnt/babylonpiles/data",
        env="DATA_DIR"
    )
    piles_dir: str = Field(
        default="/mnt/babylonpiles/piles",
        env="PILES_DIR"
    )
    temp_dir: str = Field(
        default="/tmp/babylonpiles",
        env="TEMP_DIR"
    )
    
    # Network
    wifi_ssid: str = Field(
        default="BabylonPiles",
        env="WIFI_SSID"
    )
    wifi_password: str = Field(
        default="babylonpiles123",
        env="WIFI_PASSWORD"
    )
    ...
    update_schedule: str = Field(default="0 2 * * *", env="UPDATE_SCHEDULE")  # Daily at 2 AM
    
    # Content sources
    kiwix_library_url: str = Field(
        default="https://library.kiwix.org",
        env="KIWIX_LIBRARY_URL"
    )
    osm_planet_url: str = Field(
        default="https://planet.openstreetmap.org",
        env="OSM_PLANET_URL"
    )
